    ...
    <el-main>
      ...
      <div class="info">
        ...
        <div class="info-item" id="statistic">
          <el-table class="stat-list" ref="myStatTable"
          :data="myStat" stripe border>
        ...
        <div class="info-item" id="recentGame">
          <el-table class="history-list" ref="historyTable"
          :data="historyData" :default-sort = "{prop: 'date', order: 'descending'}" stripe>
            <el-table-column prop="no" label="방번호" width="100"></el-table-column>
            <el-table-column prop="tag" label="게임" width="100">
              <template #default="scope">
                <el-tag :type="scope.row.tag === '자유' ? 'primary' : 'danger'" disable-transitions>{{scope.row.tag}}</el-tag>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="result" label="결과" width="100">
              <template #default="scope">
                <el-tag :type="scope.row.result === 'Win' ? 'primary' : 'danger'" disable-transitions>{{scope.row.result}}</el-tag>
              </template>
            </el-table-column>
            <el-table-column prop="job" label="내 직업" width="100">
              <template #default="scope">
                <el-tag :type="scope.row.job === 'police' ? 'primary' : (scope.row.job === 'citizen' ? 'default' : 'danger')" disable-transitions>{{scope.row.job}}</el-tag>
              </template>
            </el-table-column>
          </el-table>
        </div>
        ...
        <div class="info-item" id="ranking">
          <el-table class="ranking-list" ref="rankingTable"
          :data="rankingData" stripe border>
          ...
          <el-table class="ranking-list" ref="myRankingTable"
          :data="myRanking" stripe border>
...
export default {
    ...
    data(){
      return{
        ...
        userId: "",
        userDepartment: "",
        friendList: ["김철수","김철수","김철수","김철수","김철수","김철수","김철수","김철수"],
        myStat:[
          {
            job: "총합",
            win: 163,
